...
(function(root, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = factory(
      ...
      require('./brief.js'), require('./relevance.js')
    );
  } else {
  ...
  function createPipeline(deps) {
    ...
    async function run(options) {
      ...
      try {
        ...
        const activeSpace = settings.embeddingSpace || null;
        const topicCount = (await store.getAll('topics')).length;
        if (activeSpace && activeSpace !== ollama.cfg.embeddingModel && topicCount > 0) {
          throw new Error(
            `embedding model changed from "${activeSpace}" to "${ollama.cfg.embeddingModel}", which invalidates ` +
            `${topicCount} existing topic centroid(s). Restore the previous model, or use Delete everything to start ` +
            'a new record in the new model\'s vector space.'
          );
        }
        ...
        const since = (watermarkBefore && !dwellRepairNeeded)
        ...
        const runPages = await timed('embed', async () => {
          ...
          // Pages that are mechanisms rather than subjects never enter the
          // clustering: an SSO screen or a vendor download page is a step on
          // the way somewhere, not something attention was on. Excluded here
          // rather than after labelling so they cost no embed budget, and
          // reported in the uncategorized bucket like every other exclusion.
          ...
          for (const page of allPages) {
            ...
            const utility = CTRelevance.classify(page, { extraSegments: utilitySegments });
            if (utility) {
        ...
        const registryOutcome = await timed('registry', async () => {
          ...
          const keptKeys = new Set(
            applied.membershipRows.map(m => `${m.topicId}\u0000${m.normalizedUrl}`)
          );
          const removeMemberships = [];
          for (const membership of await store.getAll('memberships')) {
            if (!runUtilityUrls.has(membership.normalizedUrl)) continue;
            if (keptKeys.has(`${membership.topicId}\u0000${membership.normalizedUrl}`)) continue;
            removeMemberships.push([membership.topicId, membership.normalizedUrl]);
          }
          counts.utilityPagesExcluded = runUtilityUrls.size;
          counts.utilityMembershipsRemoved = removeMemberships.length;
        ...
        await store.setSetting('embeddingSpace', ollama.cfg.embeddingModel);
