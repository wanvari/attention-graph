  ...
  function createPipeline(deps) {
    ...
    async function run(options) {
      ...
      const runRow = {
        ...
        heartbeat: startedAt, watermarkBefore, watermarkAfter: null,
        counts, timingsMs,
      ...
      const saveRun = () => store.put('runs', { ...runRow, heartbeat: nowFn() });
      await saveRun();
      ...
      try {
        ...
        const { events, capturesByUrl } = await timed('ingest', async () => {
          ...
          for (const url of affectedUrls) {
            ...
            const captures = (byUrl.get(url) || []).sort((a, b) => b.startedAt - a.startedAt);
        ...
        const runPages = await timed('embed', async () => {
          ...
          for (const page of allPages) {
            if (page.lastSeen < since) continue;
            const captures = (capturesByUrl.get(page.normalizedUrl) || []);
            const latestWithText = captures.filter(c => c.textLength > 0).sort((a, b) => b.startedAt - a.startedAt)[0];
          ...
          counts.pagesDeferred = needWork.length - withinBudget.size;
          ...
          for (const candidate of selected) {
            ...
            pageUpdates.push({
              ...
              embeddingVariant: candidate.captureText ? 'content' : 'title_only',
              embeddedTextLength: candidate.captureText ? candidate.captureText.length : 0
            });
          }
        ...
        const registryOutcome = await timed('registry', async () => {
          ...
          const applied = CTRegistry.applyMatches(decisions, topicsById, membershipsByKey, {
            ...cfg, runId, today, now: nowFn(), pausedDays, newTopicId: deps.newTopicId
          });
