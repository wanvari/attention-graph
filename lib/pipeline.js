  ...
  function createPipeline(deps) {
    ...
    async function run(options) {
      ...
      const runRow = {
        ...
        heartbeat: wallStartedAt, watermarkBefore: null, watermarkAfter: null,
        counts, timingsMs,
      ...
      // Setup runs inside the handler too: a failure here still records a
      // failed row instead of leaving a heartbeat-less `running` one.
      try {
        ...
        runRow.watermarkBefore = watermarkBefore;
        const priorOkRuns = (await store.getAll('runs')).filter(r => r.status === 'ok' && r.runId !== runId);
        const firstRun = priorOkRuns.length === 0;
        await saveRun();
        // A missed heartbeat is harmless; the next one or the final save wins.
        heartbeatTimer = setInterval(() => saveRun().catch(() => {}), cfg.heartbeatMs);
        ...
        const { events, capturesByUrl } = await timed('ingest', async () => {
          ...
          const captureRows = await store.byIndex('captures', 'byStartedAt', store.range(since - 90 * 1000));
          ...
          const titledUrls = new Set(visitEvents.filter(e => e.hadTitle).map(e => e.normalizedUrl));
          const byUrl = new Map();
          ...
          for (const url of affectedUrls) {
            ...
            pageRows.push({
              ...
              hadTitle: titledUrls.has(url) || Boolean(existing && existing.hadTitle)
            });
          }
        ...
        const runPages = await timed('embed', async () => {
          ...
          const touchedUrls = new Set(events.map(e => e.normalizedUrl));
          const corrections = new Map((await store.getAll('corrections')).map(row => [row.correctionId, row]));
          const candidates = [];
          const utilityUrls = new Set();
          for (const page of allPages) {
            const touched = touchedUrls.has(page.normalizedUrl);
            if (corrections.get(`page:${page.normalizedUrl}`)?.kind === 'page_membership') continue;
        ...
        // Only member vectors are needed, not the whole cache.
        const memberKeys = [...new Set(liveMemberships.map(m => memberPages.get(m.normalizedUrl)?.embeddingKey).filter(Boolean))];
        const memberRows = await store.getEmbeddings(memberKeys);
        const memberEmbeddings = new Map(memberKeys.map((key, i) => [key, memberRows[i]]).filter(([, row]) => row));
        ...
        const registryOutcome = await timed('registry', async () => {
          ...
          const liveOwnerByUrl = new Map();
          for (const m of liveMemberships) if (!liveOwnerByUrl.has(m.normalizedUrl)) liveOwnerByUrl.set(m.normalizedUrl, m);
          for (const page of clusterResult.knownPages) {
            const membership = liveOwnerByUrl.get(page.normalizedUrl);
          ...
          for (const m of finalMemberships.values()) {
            if (!finalByTopic.has(m.topicId)) finalByTopic.set(m.topicId, []);
            finalByTopic.get(m.topicId).push(m);
          }
          for (const topicId of new Set(removeMemberships.map(([id]) => id))) {
            const rows = finalByTopic.get(topicId) || [];
          ...
          for (const url of completedUrls) {
            // Nothing writes pages between the registryPages read and here.
            const page = registryPages.get(url);
            if (!page) continue;
        ...
        await timed('transitions', async () => {
          ...
          // One bounded read serves every day, rolling window and gap check
          // below. Grouping keeps the index order a per-day query returns.
          const windowDays = CTMetrics.DEFAULTS.convergenceWindowDays;
          const earliest = [CTText.addDays(daysTouched[0], -(windowDays - 1)), CTText.addDays(today, -Math.max(14, windowDays))].sort()[0];
          const visitsByDay = new Map();
          for (const visit of await store.byIndex('visits', 'byDay', store.range(earliest, today))) {
            if (!visitsByDay.has(visit.dayKey)) visitsByDay.set(visit.dayKey, []);
            visitsByDay.get(visit.dayKey).push(visit);
          }
          const visitsOn = day => (visitsByDay.get(day) || []).slice();
          const visitsBetween = (from, to) => [...visitsByDay.keys()].filter(day => day >= from && day <= to).flatMap(day => visitsByDay.get(day));
          ...
          for (const day of daysTouched) {
            ...
            const windowVisits = visitsBetween(windowStart, day);
            ...
            const commitPayload = {
              ...
              categorizedUrls: [...new Set(dayVisits.map(v => v.normalizedUrl))]
            ...
            if (day === today) {
              ...
              const yWindowVisits = visitsBetween(yWindowStart, yesterday);
              ...
              let gap = null;
              {
                ...
                let gapDays = 0;
                for (let i = 0; i < 14; i++) {
                  if (visitsOn(cursor).length) break;
                  gapDays++;
              ...
              const dayStart = new Date(year, month - 1, date).getTime();
              const dayEnd = new Date(year, month - 1, date + 1).getTime();
              for (const interval of pauseIntervals) {
                const start = Math.max(interval.start, dayStart);
                const end = Math.min(interval.end ?? nowFn(), dayEnd);
