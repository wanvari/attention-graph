...
(function(root, factory) {
  ...
  function buildRecord(snapshot, options) {
    ...
    for (const capture of (data.captures || []).slice().sort((a, b) => a.startedAt - b.startedAt)) {
      ...
      if (candidates.length) {
        ...
        events.push({
          ...
          domain: CTText.extractDomain(url), time, day: CTText.dayKeyFromMs(time),
          dwellMs: measuredMs, measured, intervals, timingMethod: intervals ? 'observed-intervals' : measured ? 'interaction-total-estimate' : 'unknown', measured, pending: true, transition: 'capture', source: capture.source || 'browser'
        });
      }
    }
    events.sort((a, b) => a.time - b.time || a.id.localeCompare(b.id));
    ...
    return { now, events, trails, custom, settings, runs, sessions: data.intent_sessions || [], overrides, ...summarize(events) };
  }
  ...
  // many topics exist. Most recently opened page wins, then stable event id.
  function attributeTimelineMinutes(events, bounds = {}) {
    const points = [], heap = [], live = new Set(), out = [];
    let sequence = 0;
    const higher = (a, b) => a.event.time > b.event.time || a.event.time === b.event.time && String(a.event.id).localeCompare(String(b.event.id)) > 0;
