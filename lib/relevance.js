...
(function(root, factory) {
  ...
  // A service dashboard is useful evidence of a status check, not by itself
  // evidence of a sustained subject. Incident reports remain content.
  function isOperationalPage(page) {
    try {
      const url = new URL(page.url || page.normalizedUrl);
      if (/\/(?:incidents?|postmortems?|reports?|blog|docs|articles)\//i.test(url.pathname)) return false;
      return /^(?:status|health)\./i.test(url.hostname) && /^\/(?:index\.html)?$/.test(url.pathname) ||
        /^(?:\/status|\/health)\/?$/.test(url.pathname) && /\b(?:status|operational|uptime)\b/i.test(page.title || '');
    } catch { return false; }
  }

  function hasGroupingEvidence(page) {
    // Background refreshes can create hundreds of history rows on a status
    // page. Repetition alone is not evidence of engagement with that page.
    return isOperationalPage(page) ? Number(page.activeMs) >= 120000 : Number(page.visitCount) >= 2 || Number(page.dwellMs) >= 120000;
  }
  ...
  return {
    ...
    classify, isUtilityPage, isOperationalPage, hasGroupingEvidence
  };
});
