...
(function(root, factory) {
  ...
  function transitions(events, bounds, pauses = []) {
    ...
    const pairs = [], ungroupedPairs = [];
    const excluded = { simultaneous: 0, gap: 0, dayBoundary: 0, reload: 0, samePage: 0, paused: 0 };
    let eligible = 0;
    for (let i = 1; i < rows.length; i++) {
      ...
      const reason = counts.get(a.time) > 1 || counts.get(b.time) > 1 ? 'simultaneous'
        : gap <= 0 || gap > 30 * MINUTE ? 'gap'
        : Text.dayKeyFromMs(a.time) !== Text.dayKeyFromMs(b.time) ? 'dayBoundary'
        : b.transition === 'reload' ? 'reload' : a.normalizedUrl === b.normalizedUrl ? 'samePage'
        : pauses.some(p => p.start <= b.time && (p.end ?? Infinity) > a.time) ? 'paused' : null;
      if (reason) { excluded[reason]++; continue; }
      eligible++;
      if (a.topicIds.length !== 1 || b.topicIds.length !== 1) { ungroupedPairs.push({ from: a, to: b }); continue; }
      pairs.push({ from: a, to: b, same: a.topicIds[0] === b.topicIds[0] });
    }
    const distinct = (rows, field) => new Set(rows.map(p => JSON.stringify([p.from[field], p.to[field]]))).size;
    return { pairs, eligible, uncovered: ungroupedPairs.length, ungroupedPairs, excluded,
      candidateCount: Math.max(0, rows.length - 1),
      uniqueUngroupedPagePairs: distinct(ungroupedPairs, 'normalizedUrl'),
      uniqueUngroupedWebsitePairs: distinct(ungroupedPairs, 'domain') };
  }
