...
(function(root, factory) {
  ...
  function buildMembershipRows(topicId, cluster, existingByKey, runId) {
    const rows = [];
    for (const page of cluster.pages) {
      ...
      rows.push({
        ...
        firstDay: existing ? (existing.firstDay < page.firstDay ? existing.firstDay : page.firstDay) : page.firstDay,
        lastDay: existing ? (existing.lastDay > page.lastDay ? existing.lastDay : page.lastDay) : page.lastDay,
        dwellMs: (existing ? existing.dwellMs : 0) + (Number(page.dwellMs) || 0),
        activeMs: (existing ? existing.activeMs : 0) + (Number(page.activeMs) || 0),
        visitCount: (existing ? existing.visitCount : 0) + (Number(page.visitCount) || 0),
        assignedRunId: runId,
  ...
  function applyMatches(decisions, topicsById, membershipsByKey, options) {
    ...
    for (const decision of decisions) {
      ...
      const clusterDwell = cluster.pages.reduce((sum, p) => sum + (Number(p.dwellMs) || 0), 0);
      ...
      if (decision.action === 'merge') {
        ...
        const updated = {
          ...
          totalDwellMs: oldDwell + clusterDwell,
        ...
        membershipRows.push(...buildMembershipRows(base.topicId, { ...cluster, matchScore: decision.score }, membershipsByKey, runId));
        ...
        membershipRows.push(...buildMembershipRows(topicId, cluster, membershipsByKey, runId));
        events.push({
