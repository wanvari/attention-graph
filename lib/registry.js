...
(function(root, factory) {
  ...
  // Similarity is sufficient for new evidence: new clusters have no URL
  // overlap by construction. Reused evidence may boost a match, but absent
  // overlap must not cap its score below the merge threshold.
  function matchScore(cluster, topic, recentUrlsByTopic, cfg) {
    ...
    let overlap = 0;
    for (const page of cluster.pages) if (recent.has(page.normalizedUrl)) overlap++;
    const share = cluster.pages.length ? overlap / cluster.pages.length : 0;
    return cos + (1 - cos) * cfg.overlapWeight * share;
  }
  ...
  function matchNewClusters(newClusters, topics, memberships, options) {
    ...
    const candidates = (topics || []).filter(t => !t.needsRebuild && (t.state === 'active' || t.state === 'dormant'));
    ...
    for (const cluster of newClusters || []) {
      ...
      const supports = match => {
        const members = cfg.memberVectorsByTopic?.get(match?.topic.topicId) || [];
        return !cfg.memberVectorsByTopic || members.length && cluster.pages.every(p =>
          members.filter(v => CTCluster.cosine(p.embedding, v) >= (cfg.memberSupportThreshold || 0.70)).length >= Math.min(2, members.length));
      };
      const support = supports(best);
      const mergeProposal = best && second && best.score >= cfg.mergeThreshold && second.score >= cfg.mergeThreshold &&
        best.score - second.score < cfg.proposalGap && support && supports(second)
        ? { topicA: best.topic.topicId, topicB: second.topic.topicId, scoreA: best.score, scoreB: second.score } : null;
      const separated = !cfg.memberVectorsByTopic || !second || best.score - second.score >= (cfg.assignMargin || 0.05);
      if (best && best.score >= cfg.mergeThreshold && support && separated) {
        const decision = {
          action: 'merge',
          cluster,
        ...
        if (mergeProposal) decision.mergeProposal = mergeProposal;
  ...
  function applyMatches(decisions, topicsById, membershipsByKey, options) {
    ...
    for (const decision of decisions) {
      ...
      // Suggestions are independent of page assignment. The production
      // ambiguity margin can require a new trail while BOTH older trails
      // have strong supporting evidence. Never merge those older trails here.
      if (decision.mergeProposal) {
        ...
        if (!proposalsSeen.has(key)) {
          ...
          events.push({ topicId: decision.mergeProposal.topicA, day: today, runId, type: 'merged',
      ...
      if (decision.action === 'merge') {
        ...
        const mergedCentroid = clusterDwell > 0 ? CTCluster.centroid(
          [base.centroid, cluster.centroid],
          [Math.max(oldDwell, 1), clusterDwell]
        ) : base.centroid;
        const updated = {
          ...base,
          centroid: mergedCentroid,
          lastActiveDay: base.lastActiveDay && base.lastActiveDay > lastDay ? base.lastActiveDay : lastDay,
          lastActiveAt: Math.max(Number(base.lastActiveAt) || 0,
            ...cluster.pages.map(p => Number(p.lastVisitAt) || CTText.dayKeyToNoonMs(p.lastDay)))
        };
        if (base.state === 'dormant' && lastDay > base.lastActiveDay) {
          const dormantDays = base.lastActiveDay
            ? CTText.diffDays(base.lastActiveDay, lastDay) - pausedDaysBetween(base.lastActiveDay, lastDay, pausedDays)
            : 0;
          updated.state = 'active';
          updated.dormantSince = null;
          events.push({ topicId: base.topicId, day: lastDay, runId, type: 'revived', detail: { dormantDays } });
        } else {
          updated.state = base.state;
        }
        ...
        const row = {
          ...
          lastActiveAt: Math.max(...cluster.pages.map(p => Number(p.lastVisitAt) || CTText.dayKeyToNoonMs(p.lastDay))),
          dormantSince: null,
