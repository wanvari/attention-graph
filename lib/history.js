...
(function(root, factory) {
  ...
  // second is an explicit automatic transition -> keep only the second.
  // A shared hostname does not establish a redirect; preserve quick browsing.
  function collapseRedirects(events, cfg) {
    ...
    for (const event of events) {
      ...
      if (
        ...
        REDIRECT_TRANSITIONS.has(event.transition)
      ) {
  ...
  // Input-conditioned visible time is an observation, not a measurement of
  // attention. Use it to conservatively lower the gap estimate. Neither
  // quantity establishes whether the person was reading or thinking.
  //
  ...
  // no longer supported by fresh input observations, and the estimate must be
  ...
  function lowerDwellByCapture(gapDwellMs, capture) {
    ...
    const neverMeasured = !capture.endedAt && activeMs === 0 && !(observedMs >= DEFAULTS.captureMinObservedMs);
  ...
  function buildVisitEvents(historyItems, visitMap, options) {
    ...
    for (const item of historyItems || []) {
      ...
      const safeUrl = CTText.sanitizeUrl(item.url);
      if (!safeUrl) continue;
      const domain = CTText.extractDomain(safeUrl);
      ...
      for (const visit of visits) {
        ...
        if (!Number.isFinite(time) || time <= 0 || time < (Number(cfg.since) || 0)) continue;
        ...
        events.push({
          ...
          title: CTText.cleanTitle(item.title, safeUrl),
          domain,
          visitTime: time,
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      current.gapDwellMs = current.dwellMs;
      ...
      if (candidates) {
        const match = candidates.filter(c => !matchedCaptures.has(c.captureId) &&
          Math.abs(Number(c.startedAt) - current.visitTime) <= cfg.captureMatchWindowMs
        ).sort((a, b) => Math.abs(a.startedAt - current.visitTime) - Math.abs(b.startedAt - current.visitTime))[0];
        if (match) {
          matchedCaptures.add(match.captureId);
