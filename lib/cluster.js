...
(function(root, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = factory(require('./text.js'), require('./relevance.js'));
  } else {
    root.CTCluster = factory(root.CTText, root.CTRelevance);
  }
  ...
  function isValidVector(vector) {
    ...
    return Number.isFinite(sumSq) && sumSq > 0;
  }
  ...
  function cosine(a, b) {
    ...
    return Number.isFinite(denom) && denom > 0 ? CTText.clamp(dot / denom, -1, 1) : 0;
  }
  ...
  function centroid(vectors, weights) {
    ...
    for (let i = 0; i < vectors.length; i++) {
      if (isValidVector(vectors[i])) valid.push(i);
    }
    if (!valid.length) return new Float32Array(0);
    const dim = vectors[valid[0]].length;
    if (valid.some(i => vectors[i].length !== dim)) {
      throw new Error('Cannot combine different embedding dimensions');
    }
  ...
  function assignToExistingTopics(pages, topics, options) {
    ...
    const remainder = [];
    const candidates = (topics || []).filter(t => t.centroid && t.centroid.length && t.state !== 'retired' && !t.needsRebuild);
    for (const page of pages) {
      if (!CTRelevance.hasGroupingEvidence(page)) { remainder.push(page); continue; }
      ...
      const members = cfg.memberVectorsByTopic?.get(best?.topicId) || [];
      const supported = members.length > 0 && members.filter(v => cosine(page.embedding, v) >= cfg.memberSupportThreshold).length >= Math.min(2, members.length);
      if (best && bestScore >= cfg.assignThreshold && bestScore - secondScore >= cfg.assignMargin && supported) {
  ...
  function clusterNewPages(pagesInput, options) {
    const cfg = { ...DEFAULTS, ...(options || {}) };
    const eligible = pagesInput.filter(p => CTRelevance.hasGroupingEvidence(p));
    const { clusters, rejected } = agglomerate(eligible, cfg);
    const kept = [];
    const thinSingletons = pagesInput.filter(p => !CTRelevance.hasGroupingEvidence(p));
