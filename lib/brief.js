...
(function(root, factory) {
  ...
  const METRIC_DISPLAY = {
    ...
    switchesPerActiveHour: { name: 'Switch rate', unit: '/h', format: v => `${v.toFixed(1)} per estimated hour` },
    continuityShare: { name: 'Continuity', unit: '%', format: v => `${Math.round(v * 100)}%` },
    activeMs: { name: 'Estimated browsing time', unit: '', format: v => {
  ...
  function buildBrief(input) {
    ...
    if (firstRun) {
      items.push({
        kind: 'first_run',
        text: 'First analysis complete. Topic histories appear once the record has history to compare against.',
        topicIds: [],
        evidence: { day }
      });
    }
    ...
    for (const [metric, entry] of Object.entries(!includeDeviations || (metrics && metrics.lowData) ? {} : zScores)) {
      ...
      if (created.length) {
        ...
        items.push({
          ...
          text: created.length === 1
            ? `A new trail was grouped: ${names}.`
            : `${created.length} new trails were grouped: ${names}.`,
