// IndexedDB wrapper for the `cognitive-trails` database (schema version 6).
...
(function(root, factory) {
  ...
  // Derived projections only. Full source rows and model inputs stay intact.
  const PROJECTIONS = { captures: '_metadata_captures', topics: '_metadata_topics', embeddings: '_metadata_embeddings' };
  const REVISIONS = '_snapshot_revisions';
  const metadata = ({ extractedText, centroid, vector, ...row }) => row;
  ...
  function createStore(options) {
    ...
    let dbPromise = null;
    const stats = { transactions: 0, snapshotTableReads: 0 };
    let snapshotCache = null;
    let migrationError = null;
    // Both migrations are gated by settings rows, so once they have run on a
    // connection they are not due again until a wipe or reconnect. Every
    // worker message calls open(); this keeps that to one database handshake.
    let migrated = null;
    ...
    function openRaw() {
      return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = event => {
          ...
          if (!db.objectStoreNames.contains(REVISIONS)) db.createObjectStore(REVISIONS, { keyPath: 'table' });
          for (const [name, projection] of Object.entries(PROJECTIONS)) {
            if (db.objectStoreNames.contains(projection)) continue;
            const target = db.createObjectStore(projection, { keyPath: SCHEMA[name].keyPath });
            // Cursor backfill bounds temporary text/vector memory; upgrade is
            // atomic, and never edits/deletes the original evidence.
            const cursor = request.transaction.objectStore(name).openCursor();
            cursor.onsuccess = () => {
              if (!cursor.result) return;
              target.put(metadata(cursor.result.value)); cursor.result.continue();
            };
          }
        };
        request.onsuccess = () => {
          ...
          database.onversionchange = () => { database.close(); dbPromise = null; migrated = null; };
          resolve(database);
        };
        request.onerror = () => reject(request.error);
        request.onblocked = () => { rejected = true; reject(new Error('Reload Cognitive Trails in chrome://extensions to close older database connections, then reopen this page.')); };
      });
    }

    function db() {
      if (!dbPromise) dbPromise = openRaw().catch(error => { dbPromise = null; throw error; });
      return dbPromise;
    }
    ...
    function tx(storeNames, mode, work) {
      return db().then(database => new Promise((resolve, reject) => {
        ...
        const allNames = write ? [...new Set([...names, REVISIONS, ...names.map(n => PROJECTIONS[n]).filter(Boolean)])] : names;
        const transaction = database.transaction(allNames, mode);
        const handles = {};
        const dirty = new Set();
        for (const name of names) {
          const source = transaction.objectStore(name);
          handles[name] = !write || !SCHEMA[name] ? source : new Proxy(source, {
            get(target, property) {
              if (['put', 'delete', 'clear'].includes(property)) return (...args) => {
                const request = target[property](...args);
                if (PROJECTIONS[name]) {
                  const mirror = transaction.objectStore(PROJECTIONS[name]);
                  if (property === 'put') mirror.put(metadata(args[0]));
                  else mirror[property](...args);
                }
                if (!dirty.has(name)) {
                  dirty.add(name);
                  const revisions = transaction.objectStore(REVISIONS), gate = revisions.get(name);
                  gate.onsuccess = () => revisions.put({ table: name, revision: (gate.result?.revision || 0) + 1 });
                }
                return request;
              };
    ...
    const api = {
      ...
      async open() {
        ...
        if (!migrated) {
          migrated = (async () => {
            ...
            // A failed v3 import is retried on the next open, as before.
            if (migrationError) migrated = null;
      ...
      resetStats() { stats.transactions = 0; stats.snapshotTableReads = 0; },
      ...
      // Revisions and data share one readonly transaction. Opt-in reuse is for
      // read-only UI consumers: unchanged table arrays retain their identity.
      // The default remains a fresh snapshot for exports and mutable callers.
      async readSnapshot(names, options = {}) {
        const tables = [...new Set(names)].filter(name => SCHEMA[name]);
        if (!tables.length) return {};
        const key = JSON.stringify(tables), prior = options.reuseUnchanged && snapshotCache?.key === key ? snapshotCache : null;
        let next;
        await tx([...tables.map(n => PROJECTIONS[n] || n), REVISIONS], 'readonly', handles => {
          const gate = handles[REVISIONS].getAll();
          gate.onsuccess = () => {
            const revisions = Object.fromEntries(gate.result.map(r => [r.table, r.revision]));
            const changed = tables.filter(n => !prior || (prior.revisions[n] || 0) !== (revisions[n] || 0));
            next = { key, revisions, data: changed.length ? { ...prior?.data } : prior.data };
            for (const name of changed) {
              stats.snapshotTableReads++;
              const request = handles[PROJECTIONS[name] || name].getAll();
              request.onsuccess = () => { next.data[name] = request.result.map(metadata); };
            }
          };
        });
        if (options.reuseUnchanged) snapshotCache = next;
        return next.data;
      },
      ...
      async updateEvidence(transform, options = {}) {
        ...
        await tx(names, 'readwrite', (handles, transaction) => {
          const read = () => {
            ...
            for (const name of names) {
              ...
              request.onsuccess = () => {
                ...
                if (--pending) return;
                try {
                  const before = options.skipUnchanged ? Object.fromEntries(names.map(n => [n, snapshot[n].slice()])) : null;
                  result = transform(snapshot);
                  for (const key of result.removeMemberships || []) handles.memberships.delete(key);
                  for (const table of names) {
                    const key = row => JSON.stringify(Array.isArray(SCHEMA[table].keyPath) ? SCHEMA[table].keyPath.map(k => row[k]) : row[SCHEMA[table].keyPath]);
                    const originals = new Map((before?.[table] || []).map(row => [key(row), row]));
                    for (const row of result[table] || []) {
                      const old = originals.get(key(row));
                      // Rebuilds spread the original row. Equal nested values
                      // therefore share identity; changed vectors still write.
                      if (!old || Object.keys(old).length !== Object.keys(row).length || Object.keys(row).some(k => !Object.is(old[k], row[k]))) handles[table].put(row);
                    }
                  }
          ...
          if (options.skipIfSetting) {
            const { key, value } = options.skipIfSetting;
            const gate = handles.settings.get(key);
            gate.onsuccess = () => { if (gate.result?.value !== value) read(); };
          } else read();
        }).catch(error => { throw failure || error; }); // report why it aborted
      ...
      async retentionSweep(nowMs, retentionMs) {
        ...
        // Only captures older than the cutoff, not every retained text.
        const captures = KeyRange ? await this.byIndex('captures', 'byStartedAt', this.range(undefined, cutoff, false, true)) : await this.getAll('captures');
      ...
      // Run history is diagnostic and otherwise grows with every hourly
      // attempt. Keep the newest rows plus the newest successful run, which
      // scheduling uses to decide when the next run is due.
      async pruneRuns(keep = 200) {
        let removed = 0;
        await tx('runs', 'readwrite', handles => {
          const request = handles.runs.getAll();
          request.onsuccess = () => {
            const rows = request.result.sort((a, b) => (Number(b.startedAt) || 0) - (Number(a.startedAt) || 0));
            const latestOk = rows.find(row => row.status === 'ok');
            for (const row of rows.slice(keep)) {
              if (row !== latestOk) { handles.runs.delete(row.runId); removed++; }
            }
          };
        });
        return removed;
