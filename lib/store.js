...
(function(root, factory) {
  ...
  function createStore(options) {
    ...
    const api = {
      ...
      async registryCommit({ topics, memberships, events }, testOptions) {
      ...
      async dayCommit(day, { metrics, transitions, uncategorized, baselines, brief }) {
        ...
        return tx(stores, 'readwrite', handles => {
          ...
          if (transitions) {
            // Replace the day's rows wholesale: recomputation is authoritative.
          ...
          if (uncategorized) {
            const range = KeyRange.bound([day, ''], [day, '￿']);
            handles.uncategorized.delete(range);
