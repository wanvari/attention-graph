...
(function(root, factory) {
  ...
  function clusterPayload(cluster, index) {
    return {
      id: `c${index}`,
      ...
      pages: cluster.pages.slice(0, DEFAULTS.pagesPerCluster).map(page => ({
        title: page.title,
        url: formatUrl(page.url || page.normalizedUrl),
        text: String(page.text || '').slice(0, DEFAULTS.textCharsPerPage)
      }))
    };
  }
  ...
  async function labelClusters(clusters, client, options) {
    ...
    for (let i = 0; i < labeled.length; i += cfg.labelBatchSize) {
      ...
      const payload = batch.map((cluster, j) => clusterPayload(cluster, i + j));
      let data;
      try {
        calls++;
