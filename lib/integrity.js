...
(function(root, factory) {
  ...
  function rebuild(snapshot, now = Date.now()) {
    ...
    const audit = { version: REPAIR_VERSION, checkedAt: now, changedPages: 0, changedVisits: 0, removedMemberships: 0, changedTopics: 0, utilityMembershipsRemoved: 0,
    ...
    for (const m of snapshot.memberships || []) {
      const key = `${m.topicId}\n${m.normalizedUrl}`;
      if (!priorByKey.has(key)) priorByKey.set(key, m);
    }
    const memberships = [];
    for (const [url, evidence] of byPage) {
      const topicId = evidence.events[0].topicIds[0];
      if (!topicId) continue;
      const prior = priorByKey.get(`${topicId}\n${url}`) || {};
    ...
    const topics = (snapshot.topics || []).map(t => {
      const members = membersByTopic.get(t.topicId) || [];
  ...
  async function repair(store, options = {}) {
    return store.updateEvidence(snapshot => {
      ...
      if (options.once && settings.evidenceRepairVersion === REPAIR_VERSION) return {};
      ...
      result.settings = [{ key: 'evidenceRepairVersion', value: REPAIR_VERSION }, { key: 'evidenceAudit', value: result.audit }];
      return result;
    }, { skipUnchanged: true, ...(options.once ? { skipIfSetting: { key: 'evidenceRepairVersion', value: REPAIR_VERSION } } : {}) });
  }
