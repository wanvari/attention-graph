...
(function(root, factory) {
  ...
  function rebuild(snapshot, now = Date.now()) {
    ...
    const audit = { version: 1, checkedAt: now, changedPages: 0, changedVisits: 0, removedMemberships: 0, changedTopics: 0, utilityMembershipsRemoved: 0,
    ...
    const topics = (snapshot.topics || []).map(t => {
      ...
      if (removedTopics.has(t.topicId)) {
        if (!members.length) result.state = 'retired';
          ...
          if (vectors.length && vectors.every(v => v.length === vectors[0].length)) result.centroid = Cluster.centroid(vectors, weights).buffer;
  ...
  async function repair(store, options = {}) {
    return store.updateEvidence(snapshot => {
      ...
      if (options.once && settings.evidenceRepairVersion === 1) return {};
      ...
      result.settings = [{ key: 'evidenceRepairVersion', value: 1 }, { key: 'evidenceAudit', value: result.audit }];
      return result;
    });
  }
