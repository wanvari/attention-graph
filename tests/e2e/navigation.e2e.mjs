...
try{
  ...
  for(const label of ['Your trails','Graph','Explore','Settings','Record & privacy','Home','Graph','Home']){
    ...
    await page.getByRole('link',{name:label,exact:true}).click();await page.waitForURL(new URL(href,`${origin}/ui/`).href);
  ...
  await page.getByRole('link',{name:'Settings',exact:true}).click();await page.waitForURL(`${origin}/ui/options.html`);await page.evaluate(()=>window.ctLastNavigationTransition?.finished);
  assert.equal(await page.locator('body').evaluate(n=>getComputedStyle(n).backgroundColor),'rgb(245, 246, 242)');
  ...
  const nav=page.getByRole('link',{name:'Graph',exact:true});await nav.hover();await page.waitForTimeout(300);
  ...
  await page.getByRole('link',{name:'Your trails',exact:true}).hover();await page.waitForTimeout(60);
