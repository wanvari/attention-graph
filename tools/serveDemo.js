...
const port = Number(process.argv[2]) || Number(process.env.PORT) || 8912;
const TYPES = {
...
http.createServer((req, res) => {
  ...
  const allowed = /^\/(?:ui|lib|vendor|vendor\/fonts)\/[a-zA-Z0-9._-]+$/.test(target) || target === '/fixtures/current/snapshot.json' || /^\/design\/prototype\/[a-zA-Z0-9._-]+$/.test(target) || /^\/validation\/[a-zA-Z0-9_-]+\.(?:json|md)$/.test(target) || /^\/icon[0-9]+\.png$/.test(target);
