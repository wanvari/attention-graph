...
http.createServer((req, res) => {
  ...
  const allowed = /^\/(?:ui|lib|vendor)\/[a-zA-Z0-9._-]+$/.test(target) || target === '/fixtures/current/snapshot.json' || /^\/design\/prototype\/[a-zA-Z0-9._-]+$/.test(target) || /^\/validation\/[a-zA-Z0-9_-]+\.(?:json|md)$/.test(target) || /^\/icon[0-9]+\.png$/.test(target);
